...
if [ "$ROUTER_NAME" == "hsl" ]; then
    ...
    MAX_WAIT=30
    URL="http://$IP:8080/otp/routers/default/plan?fromPlace=48.79058074474321%2C9.177703857421873&toPlace=48.59114254808809%2C8.872661590576172&mode=TRANSIT%2CWALK&maxWalkDistance=1000&arriveBy=false&wheelchair=false&locale=en"
else
    MAX_WAIT=60
    URL="http://$IP:8080/otp/routers/default/plan?fromPlace=60.19812876015124%2C24.934051036834713&toPlace=60.218630210423306%2C24.807472229003906"
fi
